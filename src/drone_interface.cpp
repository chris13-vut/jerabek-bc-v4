...
bool DroneInterface::set_mode(std::string mode) {
    ...
    mode_client_->async_send_request(req); return true;
}
bool DroneInterface::arm(bool value) {
    auto req = std::make_shared<mavros_msgs::srv::CommandBool::Request>(); req->value = value;
    if (!arm_client_->wait_for_service(1s)) return false;
    arm_client_->async_send_request(req); return true;
}
bool DroneInterface::takeoff(double altitude) {
    auto req = std::make_shared<mavros_msgs::srv::CommandTOL::Request>(); req->altitude = altitude;
    if (!takeoff_client_->wait_for_service(1s)) return false;
    takeoff_client_->async_send_request(req); return true;
}
bool DroneInterface::land() {
    ...
    land_client_->async_send_request(req); return true;
}
